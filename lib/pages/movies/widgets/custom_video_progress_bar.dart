...
class VideoProgressBarState extends State<CustomVideoProgressBar> {
...
class _ProgressBarPainter extends CustomPainter {
  _ProgressBarPainter(this.value, this.colors, {this.stops = const []});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final stop in stops) {
      var stopPercent = stop.inMilliseconds / value.duration!.inMilliseconds;
      if (stopPercent.isNaN) {
        stopPercent = 0;
      }
      final stopPart = stopPercent > 1 ? size.width : stopPercent * size.width;

      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromPoints(
            Offset(stopPart, size.height / 2),
            Offset(stopPart + 2.5, size.height / 2 + height),
          ),
          const Radius.circular(4.0),
        ),
